...
\usepackage{ulem}
\newcommand{\roux}[2]{{\sout{#1}}{{\color{purple}~#2}}} %\renewcommand{\roux}[2]{{#1}{\phantom{#2}}}
...
Cependant, l'apprentissage et l'analyse de tels syst\`emes concurrents sont difficiles en calcul.
...
Nous proposons ensuite deux m\'ethodes d'apprentissage des mod\`eles: CRAC (Completion \textit{via} Reachability And Correlations) et M2RIT (Model Revision \textit{via} Reachability and Interpretation Transitions) en utilisant respectivement des donn\'ees continues et discr\`etes pour s'ajuster au mod\`ele et des propri\'et\'es d'accessibilit\'e pour contraindre les mod\`eles en sortie.
...
Le chapitre~\ref{chap:refinement} pr\'esente notre cadre de mod\'elisation et ses analyseurs d'accessibilit\'e li\'es bas\'es sur l'analyse statique.
...
Nos apprenants mod\`eles CRAC et M2RIT effectuent en fait une s\'election des mod\`eles.
Ils choisissent un mod\`ele parmi les candidats qui satisfont \`a toutes les contraintes d'accessibilit\'e donn\'ees.
Cependant, le nombre de mod\`eles candidats pouvant \^etre exponentiels, nos r\'eviseurs de mod\`eles peuvent r\'eduire l'espace de recherche avec des contraintes lors de la g\'en\'eration des mod\`eles.
