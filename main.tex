...
Then we propose two model learning methods: CRAC (Completion \textit{via} Reachability And Correlations) and M2RIT (Model Revision \textit{via} Reachability and Interpretation Transitions) using respectively continuous and discrete data to fit the model and using reachability properties to constrain the output models.
...
Cependant, l'apprentissage et l'analyse de tels syst\`emes concurrents sont difficiles \roux{en calcul}{pour ce qui concerne les calculs}.
...
Nous proposons ensuite deux m\'ethodes \roux{d'apprentissage}{de construction et d'apprentissage} des mod\`eles: CRAC (Completion \textit{via} Reachability And Correlations) et M2RIT (Model Revision \textit{via} Reachability and Interpretation Transitions) en utilisant respectivement des donn\'ees continues et discr\`etes pour s'ajuster au mod\`ele et des propri\'et\'es d'accessibilit\'e \roux{pour contraindre les mod\`eles en sortie}{afin de contraindre les mod\`eles résultants}.

Le chapitre~\ref{chap:intro} d\'ecrit bri\`evement le contexte et la contribution de nos recherches.
Le chapitre~\ref{chap:stateOfTheArt} pr\'esente l'\'etat de l'art des mod\'elisations, des model checkers, des diff\'erents dynamiques des syst\`emes des mod\'elisations et les techniques d'apprentissage des mod\`eles.
Certains d'entre eux sont r\'ef\'erenc\'es dans les chapitres suivants.

Le chapitre~\ref{chap:refinement} pr\'esente notre cadre de mod\'elisation et ses analyseurs d'accessibilit\'e \roux{li\'es}{associ\'es, qui sont} bas\'es sur l'analyse statique.
...
Nos \roux{apprenants mod\`eles}{syst\`emes de construction de mod\`eles par apprentissage} CRAC et M2RIT effectuent en fait une s\'election des mod\`eles.
Ils choisissent un mod\`ele parmi les candidats qui satisfont \`a toutes les contraintes d'accessibilit\'e donn\'ees.
Cependant, le nombre de mod\`eles candidats pouvant \^etre \roux{exponentiels}{de tr\`es grande taille}, nos r\'eviseurs de mod\`eles peuvent r\'eduire l'espace de recherche avec des contraintes lors de la g\'en\'eration des mod\`eles.
