...
$$\dv{x_v}{t}=k_v+\sum_{u\in v.pred}{x_uk_{uv}}-\lambda_v x_v$$
...
\begin{enumerate}
    ...
    Consider an elementary biochemical reaction $m\mathrm  {A}+n\mathrm  {B}\rightarrow \mathrm  {C}$, according to rate law, the synthesis rate of C is $r_{\mathrm  {C}}\;=\;k[{\mathrm  {A}}]^{m}[{\mathrm  {B}}]^{n}$.
...
According to Equation \ref{eq:ode}, if we consider the case $n=1$, the change rate of certain variable is the linear sum of other variables.
The linear links are detectable via an approach using Pearson correlation coefficients.

For the case $n\neq 1$, we can apply an approach using Spearman correlation coefficients.
