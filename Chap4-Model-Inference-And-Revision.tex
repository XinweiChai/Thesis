...
Take the Automata Network in Figure \ref{ExUnder} as example with initial state $\langle a_0,b_0,c_0,d_0\rangle$. Figure \ref{Under1}, Figure \ref{Under2} and Figure \ref{Under3} show the procedures of under-approximation: after two additions of actions and an update, the LCG becomes stable and $a_2$ is reachable.
\begin{figure}[!htbp]
\centering
\begin{tikzpicture}
\TSort{(3,0)}{a}{3}{l}
...
\draw[->] (O5) -- (S5);
\end{tikzpicture}
\caption[Operations on LCG(1)]{Under-approximation LCG of Figure \ref{ExUnder} studying the reachability of $a_2$. \fbox{$c_1$} unreachable as $(b_1\land c_1)=$ false, $a_2$ is not reachable (small circles stand for solutions and squares stand for processes)}\label{Under1}
\end{figure}
...
\draw[->] ($(S7)+(0,3pt)$) -- (P1);
\end{tikzpicture}
\caption[Operations on LCG(3)]{Second step: the under-approximation LCG after completion shows $a_2$ is now reachable after adding $\acm{c_1}{d_1}{d_0}$ (filled small circles stand for new possible solutions after updates)}\label{Under3}
\end{figure}
