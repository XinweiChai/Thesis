...
\begin{mybox}
Several modeling frameworks and model checking techniques \roux{are}{were} introduced in Chapter~\ref{chap:stateOfTheArt}.
...
Exact model checkers \roux{face always}{always face} the state space explosion problem when analyzing large models (of \roux{}{more than} about 50 variables);
...
This chapter is going to deepen into the reachability problem \roux{}{for Asynchronous Binary Automata Network} \textit{via} the following steps:

\begin{itemize}
    \item Why the inconclusiveness problem rises \roux{}{in our method}
    \item What are the problematic structures \roux{}{in Automata Networks or in what ?}
    \item How to deal with such structures
\end{itemize}

As a result, we try to recover the consequence of the lost \roux{}{of} information due to non-exhaustive search of static analysis and construct a more close approximation of the real dynamics in order to gain a better conclusiveness.
\end{mybox}

In this chapter, we are going to formally define the main modeling framework studied in this thesis, Asynchronous Binary Automata Network (ABAN) and \roux{its}{the} related static analyzer \roux{}{we have specifically designed}, Simplified Local Causality Graph (SLCG).
...
It starts with the target state $\omega$, \roux{look}{looks} for transitions reaching the target state, then \roux{replace}{replaces} $\omega$ with the bodies of these transitions.
...
\subsection{Reachability via Permutations} \roux{}{[PermReach]}\label{sec:permreach}
...
The experimental results are in Chapter~\ref{chap:test}\roux{}{dot . ~~~~~~~~~} Benchmark showing these two approaches can deal with more problems than pure static analysis.

Like static analysis, those two analyzers are not fully conclusive.
However, if one wants total conclusiveness, he will \roux{}{probably} need a complete search over state space like exact model checkers do, resulting in state space explosion.
