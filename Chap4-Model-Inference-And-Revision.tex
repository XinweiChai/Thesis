...
Even though there always \roux{lies an}{remains a} an non-equivalence, we wish that the abstracted models are in bisimulation with the original systems with respect to certain variables and keep the same important properties as original systems.
...
Some related works: Opgen-Rhein \textit{et als.} \roux{}{la morme exacte est : et al. aA REMPLACER PARTOUT CI-DESOUS (c'est du Latin à l'origine). Aussi, il faut unifier la position de cite[] avec have studied} have~\cite{opgen2007correlation} studied the undirectional inference \textit{via} correlation, Rodrigues \textit{et als.}~\cite{rodrigues2011active} have studied the learning of action models and Bonneau \textit{et als.}~\cite{bonneau2006inferelator} have studied the learning from continuous time-series data.
...
Cut set operation works on the local states \roux{necessitating}{mandatory} for reachability. 
...
\roux{}{Ou je ne comprends pas bien ou il y a une grosse erreur : a3, c'est QUOI, CI-DESSOUS ? de meme pour b3 etc.}
...
This association covers more orders of occurrence than over-approximation as it takes into account the mutual reachability between local Boolean states if both are \roux{appeared}{appearing} in the SLCG.
...
To sum up the whole process, we begin with \roux{a}{an} incomplete model and a set of candidate regulations.
...
The resulting program $P'$ is consistent \roux{}{with what} and realizes all previously treated transition plus $(I,J)$ \roux{}{consistent according to Def. 4.17 page 82 ????}.
...
\subsection{Revision}\label{sec:algorithm}
    ...
    \end{enumerate}
     ...
     Generalization is similar \roux{}{TO WHAT ?} but without the constraint of the observation, as the observation is partial, $P$ may describe some state transitions never observed.
    ...
    The main revision algorithm \roux{}{(see above)} starts with constructing the SLCGs to verify $Re$ and $Un$ in order to ensure the reachability/unreachability properties to be satisfied.
    ...
    \textbf{Main algorithm of revision}:
    \begin{itemize}
