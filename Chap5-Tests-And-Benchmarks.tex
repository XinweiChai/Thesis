...
PermReach and ASPReach show complete conclusiveness while Pint cannot (Figure~\ref{fig:LCG_lambdaPhage}). %able to figure out the reachability of $cro_1$ when $cl_1$ and $cll_1$ are in the initial state (Figure~\ref{fig:LCG_lambdaPhage}).
...
\begin{figure}[ht]
    ...
    \caption[Counterexample of PermReach]{The former counterexample shows PermReach returned inconclusive if multiple states of one automaton appear in the branches of one \textbf{AND gate} ($d_0$ and $d_1$ in this example). 
...
Column “Pint” gives the related results on ANs, while column “PermReach” gives the results for ABANs.}\label{tab:results}
\end{table}
...
\begin{figure}[ht]
    ...
    \subfigure[0.5\linewidth][Runtime with fixing the size to 20]{
    \centering
...
To generate such equation, we generate first $\Sigma'\subseteq \Sigma\setminus\{v\}$ randomly.
The first element of $\Sigma'$ is randomly chosen from $\Sigma\setminus\{v\}$.
For $i>1$, if $(i-1)$-th element of $\Sigma'$ exists, we generate the $i$-th element with an 80\% probability, choosing from $\Sigma\setminus(\Sigma'\cup \{v\})$.
