...
\begin{itemize}
    ...
    \item The notion of model-checking and some \roux{existed}{existing} model checkers 
...
If one wants to describes the dynamics more finely with reasonable memory use, Process Hitting (PH) framework is a good choice,  \roux{which}{which was} introduced by Paulev\'e \textit{et al.}~\cite{pauleve2011}.
...
PH is more expressive than Asynchronous Thomas' model~\cite{thomas1978} or Asynchronous BN \roux{}{[biblio reference]}. 
...
The main difference lies on the relations of the number of transitions \roux{}{that} \textit{can} be fired and the number of transitions \roux{}{that} \textit{will} be fired at given time point $t$~\cite{ribeiro2018learning,chatain2018boolean}.
...
\begin{table}[ht]
    ...
    \end{tabular}
    \caption[Update schemes]{Numbers of fireable transitions \roux{}{dans ce tableau, je ne comprends pas pourquoi min(1,m) n'est pas simplement 1 ?} in different updating scheme, where $m$ stands for the number of different variables in all the heads of fireable rule.}
    \label{tab:semantics}
\end{table}
...
to deal with \roux{software’s}{softwares leading to} infinite state space, requiring abstraction techniques to make problems tractable.
...
By \roux{inhibit}{inhibiting} these elements, we can ensure the unreachability of the desired states.
...
LFIT \roux{}{remettre biblio reference} aims at learning a logic program $P$ (definition in Section~\ref{sec:logicProgram} on page \pageref{sec:logicProgram}) from a set of state transitions $E$ in the form $S_1\to S_2$ where $S_1,S_2$ are the states of the system.
