...
\begin{algorithmic}
\State Input: an incomplete ABAN $AB$, set of candidate regulations $R$, targeted reachability $(\alpha,\omega)$
\State Output: a completed ABAN $AB'=(\Sigma, L,T)$ or $\varnothing$ if there is no solution
\State Initialization: $rev\gets\varnothing$
\State Construct over-approximate SLCG $l=SLCG(\alpha,\omega)=(V_{\mathrm{state}},V_{\mathrm{solution}}, E)$ by Algorithm \ref{AlgConstructLCG}
\If{$reach'(\alpha,\omega)=\mathbf{True}$}
    \Return{$AB$}
\EndIf
\Do
    \For{$a\in V_{\mathrm{state}}$}
        \If{$a.next=\varnothing$}
            \State $rev\gets rev\cup \{a\}$
        \EndIf
    \EndFor
    \For{$a_i\in rev$}
        \For{$(body,head,sgn)\in R$}
            \If{$head=a_i$}
                \State $rev\gets rev\setminus \{a_i\}$
            \EndIf
        \EndFor
    \EndFor
\doWhile{$rev\neq\varnothing$}

%\State Find $a\in V_{\mathrm{state}}$ s.t. $\nexists (a,sol_a)\in E$ with $sol_a\in T$
...
\begin{algorithmic}
\State Input: incomplete ABAN $AB$, set of candidate regulations $R$, targeted reachability $(\alpha,\omega)$
\State Output: a completed ABAN $AB'$ or $\varnothing$ if there is no solution
\State Initialization
\State Construct under-approximate SLCG $l=SLCG(\alpha,\omega)$ by Algorithm \ref{AlgConstructLCGUnder}
