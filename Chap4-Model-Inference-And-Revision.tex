...
\begin{definition}[$\mathsf{update}$]
\begin{equation}  
...
Algorithm~\ref{alg:cut} describes the whole process, where $\mathsf{rank}(n)$ refers to the rank of $n$ in the SLCG.
...
If we apply this Algorithm~\ref{alg:cutsetTr} to the same example, Example~\ref{example:cutset} and its cut set, resulted cut set w.r.t transitions is $\mathbb{V}_t= \{\{\acm{b_1,c_1}{}{a_1},\acm{b_0}{}{a_1}\},\{\acm{b_1,c_1}{}{a_1},\acm{d_1}{}{b_0}\},\{\acm{b_1,c_1}{}{a_1},\acm{b_1}{}{d_1}\}\}$.
...
%    sol_{a_i}\in V_{\mathrm{sol}}&\Leftrightarrow& \{ (sol_{a_i},\mathbf{V}(sol_{a_i}))| \mathbf{V}(sol_{a_i})=\varnothing \text{ \rm if } a_i\in \alpha\land a_{1-i}\not\in V_{\mathrm{state}},\\
...
\begin{example}
Given ABAN $\mathbb{A}=(\Sigma,T)$ in Figure~\ref{ExUnder} with $\Sigma= \{a,b,c,d\}$, $T=\{\acm{b_1,c_1}{a_0}{a_1}, \acm{b_1}{d_0}{d_1}, \acm{d_0}{b_0}{b_1}\}$, the initial state $\langle a_0,b_0,c_0,d_0\rangle$.
...
\begin{definition}[Asynchronous successors]
	...
	$$T_P^{as}(I,S) = \{I \setminus \{v^{val'}\} \cup \{v^{val}\} | v^{val'} \in I, v^{val} \in T_P(I,S)\} \cup \{I|T_P(I,S) = \varnothing\}$$ % Self loop only for non attractor point 
\end{definition}
...
\subsection{Revision}\label{sec:algorithm}
    ...
    \begin{itemize}
        \item \textbf{Input}: a logic program $P$, an unsatisfied element $reach(\alpha,\omega)=\mathbf{False}$, a satisfied reachable set $Re$%, a maximum addable variables $k$
    ...
    The main revision algorithm starts with constructing the SLCGs to verify $Re$ and $Un$ in order to ensure the reachability/unreachability properties to be satisfied.
