...
%\For{each $ scc=(V'_{\mathrm{state}},V'_{\mathrm{solution}}) \in \mathrm{SCC}(l)$ with $V'_{\mathrm{state}} \subseteq V_{\mathrm{state}},V'_{\mathrm{solution}}\subseteq V_{\mathrm{sol}}$}
%    \If{$scc$ has less than one incoming edge}
%        \For{each $v \in V'_{\mathrm{state}}$}
%            \If {$\exists(v,v')\in E, v' \in (V_{\mathrm{sol}} \setminus V'_{\mathrm{solution}})$}
...
%    \State $l'=(V'_{\mathrm{state}}, V'_{\mathrm{solution}},E')\gets(V_{\mathrm{state}}, V_{\mathrm{sol}},E)$
...
\For{each $(V'_{\mathrm{state}} \subseteq V_{\mathrm{state}},V'_{\mathrm{solution}} \subseteq V_{\mathrm{sol}}) \in \mathrm{SCC1}(l)$}
    \For{each $v \in V'_{\mathrm{state}}$}
    \If {$\exists(v,v')\in E, v' \in (V_{\mathrm{sol}} \setminus V'_{\mathrm{solution}})$}
        \State $E\gets E\setminus \{(v,v'')|v''\in V'_{\mathrm{solution}},(v,v'')\in E\}$
    \EndIf
...
\begin{algorithmic}
\State Input: an incomplete ABAN $\mathbb{A}=(\Sigma,T)$, set of candidate regulations $R$, targeted reachability $(\alpha,\omega)$
