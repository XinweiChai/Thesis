...
\begin{definition}[Model completion]
    ...
    \begin{itemize}
        \item  $R=\{(body,head,sgn)| body\in LS, head\in LS, sgn\in \{+,-\}\}$ 
        \item $C=\{(s,reach)|s\in LS, reach\in \{\mathbf{True,False}\}\}$
    \end{itemize}
\end{definition}
...
Before our research began, Paulev\'e \textit{et al.} have worked on cut-sets for reachability in large scale automata networks \cite{PAK13-CAV}, which is the reverse operation of completion set.
...
Over-approximation in only a necessary condition of reachability.
...
Under-approximation is another reasoning related to LCG. It associates the local states with any local states which may appear instead of the initial state. Likewise, if there is a pathway from certain local state and the initial state, this state should be reachable. However, all the orders of occurrence do not necessarily appear during the simulation, which suggests that this approach ``under-approximates'' the reachability. If under-approximation of certain state is true, then this state is certainly reachable. Nevertheless, the computation of under-approximation is more complicated than over approximation, as the set of appearing states grows during the computation, former added links need to be verified with new appearing states until the set becomes stable. This process is called ``update''. 

Take the Asynchrhonous Automata Network $AAN=(\Sigma, L, T)$ in Figure \ref{ExUnder} as example with $\Sigma= \{a,b,c,d\}$, $T=\{\acm{b_1}{a_0}{a_1}, \acm{b_1}{d_0}{d_1},\acm{c_1}{a_1}{a_2}, \acm{d_0}{b_0}{b_1}\}$, initial state $\langle a_0,b_0,c_0,d_0\rangle$.
...
\caption[Operations on LCG(1)]{Under-approximation LCG of Figure \ref{ExUnder} studying the reachability of $a_2$. \fbox{$c_1$} unreachable then the joint state $\langle b_1, c_1\rangle$ is not reachable, $a_2$ is not reachable (small circles stand for solutions and squares stand for processes)}\label{Under1}
\end{figure}

\begin{figure}[ht]
\centering
\input{figures/completionUnderLCG2.tex}
\caption[Operations on LCG(2)]{According to candidate regulation ${(d,c,-)}\in R$, \ac{d_1}{c_0}{c_1} is added, but the reachability of under-approximation requires all the possible occurrences are realizable, e.g. transition $d_1\Rsh^*d_0$ must be firable (filled small circles stand for new possible solutions after completion)}\label{Under2}
\end{figure}

\begin{figure}[ht]
\centering
\input{figures/completionUnderLCG3.tex}
\caption[Operations on LCG(3)]{Second step: the under-approximation LCG after completion shows $a_2$ is now reachable after adding $\acm{c_1}{d_1}{d_0}$ according to ${(c,d,-)}\in R$ (filled small circles stand for new possible solutions after updates)}\label{Under3}
\end{figure}
\section{Model Inference via Statistics}

In the previous sections, only the systems with delay 1 are discussed (BN, ABAN, etc.), i.e. the impact done by variables will take place at the next time point (immediately).
...
\begin{definition}[ABAN with delay]
